...
import static art.lookingup.RaveStudio.GLOBAL_FRAME_RATE;
...
abstract class PGBase extends LXPattern {
  ...
  public PGBase(LX lx, int width, int height, String drawMode) {
    ...
    if (P3D.equals(drawMode) || P2D.equals(drawMode)) {
      pg = RaveStudio.pApplet.createGraphics(width, height, drawMode);
    } else {
      pg = RaveStudio.pApplet.createGraphics(width, height);
    }
