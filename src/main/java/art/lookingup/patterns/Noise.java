...
public class Noise extends PGPixelPerfect {
  ...
  protected void draw(double deltaDrawMs) {
    ...
    xstart += 2.0 * Math.sin((RaveStudio.pApplet.noise(xstartNoise)*0.5)-0.25);
    ystart += 2.0 * Math.cos((RaveStudio.pApplet.noise(ystartNoise)*0.5)-0.25);
    ...
    for (int y = 0; y <=30.0; y+=yDensity.getValuef()) {
      ...
      for (int x = 0; x <= 420.0; x+=xDensity.getValuef()) {
        xnoise += 0.1;
        drawPointLine(x, y, RaveStudio.pApplet.noise(xnoise, ynoise));
      }
    }
  }
  ...
  protected void drawPointLine(float x, float y, float noiseFactor) {
    ...
    pg.translate(x, y);
    noiseFactor = 1.0f;
    pg.rotate(noiseFactor * RaveStudio.pApplet.radians(360) * rotateMult.getValuef());
