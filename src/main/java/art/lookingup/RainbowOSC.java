      ...
      if ("rainbow".equals(path[1])) {
        if (path.length > 2) {
          ...
          if ("mode".equals(path[2])) {
            ...
            RaveStudio.modeSelector.switchToMode(message.getString(0));
          }
          // TODO(tracy): Maybe change this to mobilesensor or something?
          if ("mobile".equals(path[2])) {
            if (path.length > 3) {
              if ("accelx".equals(path[3])) {
                RaveStudio.oscSensor.accelXKnob.setValue(message.getFloat(0));
              } else if ("accely".equals(path[3])) {
                RaveStudio.oscSensor.accelYKnob.setValue(message.getFloat(0));
              } else if ("accelz".equals(path[3])) {
                RaveStudio.oscSensor.accelZKnob.setValue(message.getFloat(0));
              }
            }
          }
        }
